...
def show_asst_resp(chat_resp: Response):
    md = []
    if len(chat_resp.reasoning.strip()) != 0:
        md.append("##### Reasoning")
        md.append(chat_resp.reasoning)

    md.append("##### Response")
    md.append(chat_resp.resp)
...
st.set_page_config(page_icon="💬", layout="wide", page_title="ACM Assistant", initial_sidebar_state="collapsed")
...
If you have any questions, suggestions, or new feature requests,
please log them on the https://github.com/skeeey/acm-troubleshooter/issues
"""
...
if "messages" not in st.session_state:
    st.session_state.messages = [
        {"role": "assistant", "content": f"Hi, {user.name}, what can I help with?"}
    ]
