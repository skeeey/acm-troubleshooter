...
from server.tools.llm import respond, convert_question
from server.workflows.self_rag.state import copy_state

logger = logging.getLogger(__name__)

def retrieve_func(vector_store_svc: VectorStoreService):
    def retrieve(state):
        ...
        nodes = vector_store_svc.retrieve(query=query, sources=sources)
        if len(nodes) == 0:
            ...
            nodes = vector_store_svc.retrieve(query=new_query, sources=sources)
            if len(nodes) == 0:
